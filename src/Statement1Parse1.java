...
public final class Statement1Parse1 extends Statement1 {
    ...
    private static void parseIf(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(Tokenizer.isCondition(tokens.front()),
                "Error: Missing valid 'IF' condition.");
        Condition con = parseCondition(tokens.dequeue());
        Reporter.assertElseFatalError(tokens.front().equals("THEN"),
                "Error: Missing 'THEN'.");
        tokens.dequeue();
        Statement ifBodyCon = s.newInstance();
        ifBodyCon.parseBlock(tokens);
        Reporter.assertElseFatalError(
                tokens.front().equals("ELSE") || tokens.front().equals("END"),
                "Error: Missing 'ELSE' or 'END'.");
        if (tokens.front().equals("ELSE")) {
            tokens.dequeue();
            Statement elseBodyCon = s.newInstance();
            elseBodyCon.parseBlock(tokens);
            s.assembleIfElse(con, ifBodyCon, elseBodyCon);
        } else {
            s.assembleIf(con, ifBodyCon);
        }
        Reporter.assertElseFatalError(tokens.front().equals("END"),
                "Error: Missing 'END'.");
        tokens.dequeue();
        Reporter.assertElseFatalError(tokens.front().equals("IF"),
                "Error: Missing 'IF' after 'END'.");
        tokens.dequeue();
    ...
    private static void parseWhile(Queue<String> tokens, Statement s) {
        ...
        tokens.dequeue();
        Reporter.assertElseFatalError(Tokenizer.isCondition(tokens.front()),
                "Error: Missing valid 'WHILE' condition");
        Condition con = parseCondition(tokens.dequeue());
    ...
    public void parse(Queue<String> tokens) {
        ...
        Reporter.assertElseFatalError(
                tokens.front().equals("IF") || tokens.front().equals("WHILE")
                        || Tokenizer.isIdentifier(tokens.front()),
                "Error: Missing a 'IF', 'WHILE', or Identifer.");
    ...
    public void parseBlock(Queue<String> tokens) {
        ...
        while (!tokens.front().equals("END") && !tokens.front().equals("ELSE")
            ...
            this.parse(tokens);
            int pos = s.lengthOfBlock();
            s.addToBlock(pos, this);
        }
        this.transferFrom(s);
    }
